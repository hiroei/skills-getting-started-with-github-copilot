document.addEventListener("DOMContentLoaded", () => {
             ...
             <ul class="participants-list">
               ${details.participants.map((email) => `
                 <li class="participant-item">
                   <span>${email}</span>
                   <button class="participant-remove" data-activity="${name}" data-email="${email}" aria-label="Unregister ${email}">✕</button>
                 </li>
               `).join("")}
             </ul>
           </div>`
  ...
  activitiesList.addEventListener("click", async (event) => {
    const removeButton = event.target.closest("button.participant-remove");
